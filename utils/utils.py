...
import logging
logging.basicConfig(filename="logs/common_log.log",filemode='a', format='%(asctime)s,%(msecs)d %(name)s %(levelname)s %(message)s', datefmt='%H:%M:%S', level=logging.DEBUG)
...
def get_transformation():
	...
	return transforms.Compose([transforms.RandomResizedCrop(224),transforms.RandomHorizontalFlip(),transforms.ToTensor(),transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
...
def get_data_loaders(input_dir, batch_size=10,num_workers=6,val_size=100,test_size=100):
	...
	val_dataset = ImageDataset(input_dir=input_dir,test_size=test_size,val_size=val_size,transform=data_transforms,val=True)
	test_dataset = ImageDataset(input_dir=input_dir,test_size=test_size,val_size=val_size,transform=data_transforms,train=False)
